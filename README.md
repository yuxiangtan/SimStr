...
In this case, all the Path_to_SimStr in the following guideline could be replaced by /script/
...
## 1. Generate the list of candidate strains (with paired WGS data) of a specific species:
  ...
  Usage: sh Path_to_SimStr/SIM_str_ANI_PAN_mash.sh sra_result.csv SraRunInfo.csv strs.csv fileout_header /Path_to_SimStr/ range_type min_ANI
    ...
    Required: (all of these files need to be download manually)
    	...
    	Path_to_SimStr: the full path of SimStr scripts folder
...
```shell
python Path_to_SimStr/Strain_list_merger.py -i benchmark_comp_ART_1x.csv -D Path_to_SimStr --sf Path_to_ART/art_src_MountRainier_Linux/ -o outfolder
```

​	Path_to_ART is the path where the art_illumina command is
...
```
Rscript Path_to_SimStr/SIM_evaluate_across_methods.R 
stat_all_list_f=stat_all_list_f metadata_f=metadata_f stat_all_list_f=stat_all_list_f dis_all_list_f=dis_all_list_f RAD_all_list_f=RAD_all_list_f RA_all_list_f=RA_all_list_f Ab_all_list_f=Ab_all_list_f AD_all_list_f=AD_all_list_f out_name=outheader
```
