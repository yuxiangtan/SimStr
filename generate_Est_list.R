#usage example: Rscript Path_to_SimStr/generate_Est_list.R wfd=folder_fq_files fq_reg=recognizer of the first end fq file(for example _1.fq)
...
Est_list <- function(f_path,fq_reg){
  ...
  write.table(out_list, file="EST_files.csv",sep=",", quote=F, row.names= FALSE, col.names= FALSE )
  return(out_list)
}

outfile <- Est_list(f_path,fq_reg)